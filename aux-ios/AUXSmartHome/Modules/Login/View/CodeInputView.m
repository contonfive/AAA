...
- (void)initSubviews {
    ...
    if (@available(iOS 12.0, *)) {
        self.textView.textContentType = UITextContentTypeOneTimeCode;
    }
    //默认编辑第一个.
    [self beginEdit];
    for (int i = 0; i < _inputNum; i ++) {
        ...
        subView.layer.borderColor = [[UIColor colorWithHexString:@"EEEEEE"] CGColor];

        subView.frame = CGRectMake(Padd+(K_W+Padd)*i, 0, K_W, H);
        subView.userInteractionEnabled = NO;
        [self addSubview:subView];
        [CALayer addSubLayerWithFrame:CGRectMake(0, H-2, K_W, 2) backgroundColor:[UIColor clearColor] backView:subView];
        //Label
