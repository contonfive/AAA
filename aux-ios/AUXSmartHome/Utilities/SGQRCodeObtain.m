...
- (void)establishQRCodeObtainScanWithController:(UIViewController *)controller configure:(SGQRCodeObtainConfigure *)configure {
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied){
        [self alertWithMessage:@"非常抱歉，同意相机权限才能正常扫码" confirmTitle:@"去设置" confirmBlock:^{
    ...
    configure.sessionPreset = [self set_capture_present:self.device];
    ...
    videoPreviewLayer.frame = self.videoPreview.frame;
...
- (void)setVideoScale:(CGFloat)scale
{
    ...
    self.device.videoZoomFactor = scale;
...
- (UIView *)videoPreview {
    if (!_videoPreview) {
        UIView *videoView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.controller.view.frame), CGRectGetHeight(self.controller.view.frame))];
        videoView.backgroundColor = [UIColor clearColor];
        [self.controller.view addSubview:videoView];
...
#pragma mark  判断设备支持的分辨率
- (NSString *)set_capture_present:(AVCaptureDevice*)device{
    NSArray * supportsArray = @[AVCaptureSessionPreset3840x2160,
    ...
    for (NSString *tmpStr in supportsArray) {
        if ([device supportsAVCaptureSessionPreset:tmpStr]) {
            return tmpStr;
        }
    }
    return nil;
